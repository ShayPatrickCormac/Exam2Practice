...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        space_available = self.volume - len(self.contents)
        number_of_character_to_append = min(space_available,len(additional_contents))
        stuff_to_add = ''
        for k in range(number_of_character_to_append):
    ...
    def double(self):
        ...
        addtional_contents = self.append_string(self.contents)
        return addtional_contents
    ...
    def shrink(self, new_volume):
        ...
        append = len(self.contents) - new_volume
        if append <= 0:
            return ''
        if append > 0:
            new_string = ''
    ...
    def double_then_shrink(self, new_volume):
        ...
        length1 = len(self.double())
        length2 = len(self.shrink(new_volume))
